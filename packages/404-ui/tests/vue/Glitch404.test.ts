...
describe("Glitch404 (vue)", () => {
  it("mounts without crashing", () => {
    const wrapper = mount(Glitch404);
    expect(wrapper.element).toBeTruthy();
    wrapper.unmount();
  });

  it("renders custom title/subtitle/buttonText", () => {
    const wrapper = mount(Glitch404, { props: CUSTOM_PROPS });
    expect(wrapper.find("h1").text()).toBe(CUSTOM_PROPS.title);
    expect(wrapper.text()).toContain(CUSTOM_PROPS.subtitle);
    expect(wrapper.find("button.glitch-404-button").text()).toContain(
  ...
  it("emits buttonClick when button clicked", async () => {
    const wrapper = mount(Glitch404, { props: CUSTOM_PROPS });
    await wrapper.find("button.glitch-404-button").trigger("click");
